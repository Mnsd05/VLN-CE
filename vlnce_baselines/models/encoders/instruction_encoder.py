...
#         # Initialize the RNN (GRU or LSTM) based on the configuration
...
#         # Initialize the embedding layer if the sensor UUID is "instruction"
...
#         # Calculate lengths of the instructions.
#         # This is done by checking for non-zero elements in the instruction tensor.
#         # If the instruction is 3D (batch x seq x dim), we sum over the last dimension to get non-zero vectors,
#         # and then sum over the sequence dimension to get the length.
